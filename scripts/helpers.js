...
hexo.extend.helper.register('local_link', function(path) {
  // const lang = this.page.lang;
...
hexo.extend.helper.register('plugin_list', function() {
  ...
  if (type === 'blogs') {
    arr.forEach(blog => {
      // Convert the date field into a timestamp


      if (blog.date) {
        blog.timestamp = moment(blog.date, 'MMM D, YYYY').valueOf(); // Example: "FEB 3, 2024"
        // console.log(blog.timestamp);
      } else {
        blog.timestamp = 0; // Default to zero if no valid date
        // console.log(blog.timestamp);
      }
    });
